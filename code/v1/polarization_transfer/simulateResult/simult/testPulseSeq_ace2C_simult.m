...
J             = 127;                     % coupling in Hz
B1_range_H    = linspace(0.8,1.2,10);%pars.B1_range;           % pulse amplitude range for compensation
B1_range_X    = linspace(0.7,1.3,10);%pars.B1_range;           % pulse amplitude range for compensation
fRange        = linspace(-25,25,15);%pars.fRange;             % offresonance range for compensation
min_C_amp     = pars.min_X_amp;
min_H_amp     = pars.min_H_amp;
C_maxAmp      = pars.X_maxAmp;
...
T1_I = 7e9;% pars.T1_I;         % Xnuc longitudinal
T2_I = 1.6e9;%pars.T2_I;         % Xnuc transverse
T1_S = 2e9;%pars.T1_S;         % 1H longitudinal
T2_S = .4e9;%pars.T2_S;         % 1H transverse
% relaxation rates
...
%% initialization
initState = Ix - 1i*Iy;
traceNorm = trace(initState'*initState);
coil      = Sx - 1i*Sy;
%% fitness evaluation
sum_weight = 0;
...
for ff = 1:length(fRange)
    ...
    for B1_C = B1_range_X
        ...
        for B1_H = B1_range_H
            ...
            WBC         = B1weight_X(bb_C);
            weight      = WBC*WBH*Wf;
            signal_tmp           = trace(coil'*rho)/sqrt(trace(coil'*coil))/sqrt(trace(initState'*initState));
