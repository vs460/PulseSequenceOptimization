...
J_CH          = 140;                    % coupling in Hz
J_HH3         = 7.1;                     % coupling in Hz
B1_range_X    = linspace(1,1,1);           % pulse amplitude range for compensation
B1_range_H    = linspace(0.01,3,63);          % pulse amplitude range for compensation
fRange        = -100:10:100;               % offresonance range for compensation
...
avgTransfer = 1e3*gather(-abs(signal_tred));

%% detection
rho         = reshape(rho_ideal,[size(initState)]);
peakTranfer = trace(coil'*rho)/traceNorm
nsteps      = 4096;
BW          = 4000;
