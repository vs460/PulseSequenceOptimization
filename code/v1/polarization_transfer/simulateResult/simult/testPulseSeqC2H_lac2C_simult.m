...
B1_range_H    = linspace(0.1,3,20);%pars.B1_range;           % pulse amplitude range for compensation
B1_range_X    = linspace(0.1,3,20);%pars.B1_range;           % pulse amplitude range for compensation
fRange        = linspace(-400,400,50);%pars.fRange;             % offresonance range for compensation
...
signal_Z     = zeros(lB1r_X,lB1r_H,lB0r);

h = waitbar(0,'Evaluation over grid');
for ff = 1:length(fRange)
    Wf      = lineBroadening(ff);
    df      = fRange(ff);
    H_free  = 2*pi*df*offres + 2*pi*Jcoupling;% + ...                %frequency offset
              %2*pi*metOffset*Sz_CH3;
    bb_C    = 1;
    for B1_C = B1_range_X
        parfor bb_H = 1:length(B1_range_H)
            B1_H = B1_range_H(bb_H)
            WBH  = B1weight_H(bb_H);
            WBC  = B1weight_X(bb_C);
            rho  = initState(:);
            %%%%%%%%%% Pulse sequence %%%%%%%%%%
            for j = 1:nvar
                ...
                %%% 1H-Xnuc pulse %%%
                H   = 2*pi*gamma_H*B1_S*(cos(ph_S)*Sx_CH + sin(ph_S)*Sy_CH)+...
                ...
                if (df == min(abs(fRange)) && abs(B1_C-1) == min(abs(B1_range_X-1)) && abs(B1_H-1) == min(abs(B1_range_H-1)))
                    rho_ideal = rho;
                end
            end
            ...
            signal_Z(bb_C,bb_H,ff) = trace(Sz_CH'*rho)/traceNorm;
            sum_weight           = sum_weight + weight;
        end
        waitbar(((ff-1)*lB1r_H*lB1r_X+(bb_C-1)*lB1r_H+lB1r_H)/(lB0r*lB1r_H*lB1r_X))
        bb_C = bb_C + 1;
    end
end
pars.signal = signal;
