...
methyleOffset = 2.79*299.709;
B1_range_H    = linspace(.9,1.1,3);%pars.B1_range;           % pulse amplitude range for compensation
B1_range_X    = linspace(.9,1.1,3);%pars.B1_range;           % pulse amplitude range for compensation
fRange        = linspace(-5,5,3);%pars.fRange;             % offresonance range for compensation
min_N_amp     = pars.min_X_amp;
min_C_amp     = pars.min_H_amp;
...
T1_I = 50;%pars.T1_I;         % Xnuc longitudinal
T2_I = 1.6;%pars.T2_I;         % Xnuc transverse
T1_S = 1.7;%pars.T1_S;         % 1H longitudinal
T2_S = 1.863;%pars.T2_S;         % 1H transverse
% relaxation rates
...
for ff = 1:length(fRange)
    ...
    for B1_C = B1_range_X
        ...
        for B1_H = B1_range_H
            ...
            signal(bb_C,bb_H,ff) = signal_tmp*weight;
...
H_free       = 2*pi*methyleOffset*Sz_CH + ...
               2*pi*Jcoupling;
