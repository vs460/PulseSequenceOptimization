function[fitness] = polTrC2H_lac1C_approx_simult(population,pars)
tic
...
B1_range      = pars.B1_range;           % pulse amplitude range for compensation
...
B1weight = normpdf(B1_range-1,0,0.5/2.355);
        ...
        for B1_C = B1_range_X
            ...
            for B1_H = B1_range_H
                ...
                for j = 1:5:nvar
                    ...
                    ph_S = parameters(j+1);
                    B1_I = min(round(abs(parameters(j+2))/min_C_amp),1)*sign(parameters(j+2))*B1_C*max(min(abs(parameters(j+2)),pars.X_maxAmp),min_C_amp);
                    ph_I = parameters(j+3);
