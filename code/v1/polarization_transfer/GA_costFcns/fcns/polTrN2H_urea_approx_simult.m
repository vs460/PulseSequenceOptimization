function[fitness] = polTrN2H_urea_approx_simult(population,pars)
...
parfor i = 1:npop
    ...
    sum_weight = 0;
    rem_signal_array  = zeros(lB1r_X,lB1r_H,lB0r);
    tred_signal_array = zeros(lB1r_X,lB1r_H,lB0r);
        ...
        for B1_C = B1_range_X
            ...
            for B1_H = B1_range_H
                ...
                for j = 1:5:nvar
                    ...
                    H    = 2*pi*B1_I*(cos(ph_I)*Ix + sin(ph_I)*Iy)+...    
                           2*pi*B1_S*(cos(ph_S)*Sx + sin(ph_S)*Sy)+...        
                           H_free;                                                          
                    PH   = expm(-1i*H*tau);
                    PHt  = PH';
                ...
                WBC = B1weight_X(bb_C);
                weight      = Wf*WBH*WBC;
                rem_signal  = trace(Iz'*rho)/traceNorm;
                tred_signal = trace(coil'*rho)/traceNorm;
                tred_signal_array(bb_C,bb_H,ff) = weight*tred_signal;
                rem_signal_array(bb_C,bb_H,ff)  = weight*rem_signal;
    ...
    signal_rem_diff = abs(signal_rem-presPol);
    fitness(i)	= 1e3*gather(signal_rem_diff-abs(signal_tred));
end
toc
end
