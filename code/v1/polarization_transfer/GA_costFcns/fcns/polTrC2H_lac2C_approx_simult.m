function[fitness] = polTrC2H_lac2C_approx_simult(population,pars)
tic
...
parfor i = 1:npop
    ...
    % enforce bound constraint on the pulse amplitude    
    ...
    for ff = 1:length(fRange)
        ...
        if abs(df-10245)<500 || abs(df-1914)<500
            H_free = 2*pi*df*Iz;
        else
            H_free = 2*pi*df*offres + 2*pi*Jcoupling;% + ...                
          %%%%%% Uncomment if all 4 protns acconted for %%%%%%%%%
        ...
        for B1_C = B1_range_X
            ...
            for B1_H = B1_range_H
                ...
                for j = 1:5:nvar
                    ...
                    ph_S = parameters(j+1);
                    ph_I = parameters(j+3);
                    tau  = min(max(0,parameters(j+4)),1/abs(J_CH));
                    
                    %%% 1H-Xnuc pulse %%%
                    H = 2*pi*B1_S*(cos(ph_S)*Sx + sin(ph_S)*Sy)+...    %pulse on proton
                        2*pi*B1_I*(cos(ph_I)*Ix + sin(ph_I)*Iy)+...
                        H_free;                                                          %coupling
                    PH  = expm(-1i*H*tau);
                    PHt = PH';
    ...
    signal_tred = sum(sum(sum(sum(signal))))/sum_weight;
    % Penalty to enforce frequency selectivity, i.e. zero excitation on
    % pyvate and PH resonances
    cost = calcRFprofileNMRstuff(B1_I_vec,parameters,taus_tmp,pars,0);
    fitness(i)	= 1e3*gather(-abs(signal_tred)+cost);
end
toc
end
