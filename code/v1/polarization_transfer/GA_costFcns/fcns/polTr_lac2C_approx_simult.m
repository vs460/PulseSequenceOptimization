function[fitness] = polTr_lac2C_approx(population,pars)
tic
...
parfor i = 1:npop
    ...
    taus_tmp   = min(max(1e-17,parameters(5:5:end)),1/abs(J_CH));
    T          = cumsum(taus_tmp);
        ...
        for B1_C = B1_range_X
            ...
            for B1_H = B1_range_H
                ...
                for j = 1:5:nvar
                    ...
                    ph_S = parameters(j+1);
                    ph_I = parameters(j+3);
                    tau  = taus_tmp((j-1)/5+1);
                    
                    %%% 1H-Xnuc pulse %%%
                    H = 2*pi*B1_S*(cos(ph_S)*Sx + sin(ph_S)*Sy)+...    
