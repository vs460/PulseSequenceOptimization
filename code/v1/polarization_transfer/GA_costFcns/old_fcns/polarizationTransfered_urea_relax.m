function[fitness] = polarizationTransfered_urea_relax(population,pars)
tic
...
if ~genH_sup
    ...
    load(fullfile(path,'H_sup_Sx.mat'));
    load(fullfile(path,'H_sup_Sy.mat'));
    load(fullfile(path,'H_sup_Sz.mat'));
    load(fullfile(path,'H_sup_IzSz.mat'));
...
J = -90;
B1_range = 0.2:0.1:1.8;                  % pulse amplitude range for compensation
fRange = -50:5:50;                   % offresonance range for compensation
...
T1_I = 24;                         % 13C longitudinal
T2_I = 1.62;                        % 13C transverse
T1_S = 2.67;                       % 1H longitudinal
...
% permutation_mtx(1,:) = [1,1,1];
% permutation_mtx(2:4,:) = [2,1,1;3,1,1;4,1,1];
% permutation_mtx(5:7,:) = circshift([2,1,1;3,1,1;4,1,1],[0,1]);
% permutation_mtx(8:10,:) = circshift([2,1,1;3,1,1;4,1,1],[0,2]);
...
for bb = 1:4^nspins
    ...
    % normalization 
    B(bb) = {basis_prod_tmp*2^(sum(permutation_mtx(bb,:)>1)-1)/sqrt(2)};
end
%B = cell((2^nspins)^2,1);
...
if genH_sup
    ...
    close(h)
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Ix','H_sup_Ix')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Iy','H_sup_Iy')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Iz','H_sup_Iz')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Sx','H_sup_Sx')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Sy','H_sup_Sy')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_Sz','H_sup_Sz')
   save('C:\Users\Somai01\Documents\NMR_stuff\INEPT_optimization\matlabDATAfiles\urea\H_sup_IzSz','H_sup_IzSz')
end
...
for i = npop:-1:1
    ...
    remaining_signal = zeros(length(B1_range),length(B1_range),length(fRange));
    signal_tmp = 0;
    sum_weight = 0;
    spurious_signal = 0;
        ...
        for B1_N = 1%B1_range
            ...
            for B1_H = B1_range
                ...
                for j = 1:5:nvar
                    ...
                    B1_I = min(round(abs(parameters(j+2))/min_N_amp),1)*sign(parameters(j+2))*B1_N*max(min(abs(parameters(j+2)),pars.X_maxAmp),min_N_amp);
                    ...
                    H_sup = 2*pi*gamma_H*B1_S*(cos(pulse_phase_S)*H_sup_Sx + sin(pulse_phase_S)*H_sup_Sy) + ...  %pulse on proton
                            2*pi*gamma_N*B1_I*(cos(pulse_phase_I)*H_sup_Ix + sin(pulse_phase_I)*H_sup_Iy) + ...  %pulse on proton
                ...
                signal_tmp = (rho(5)+1i*rho(9)+rho(17)+1i*rho(33));
                weight = WBN*WBH*Wf;
                signal(bb_N,bb_H,ff) = signal_tmp;
                remaining_signal(bb_N,bb_H,ff) = rho(4);
    ...
    remaining_signal = sum(sum(sum(remaining_signal)))/sum_weight;
