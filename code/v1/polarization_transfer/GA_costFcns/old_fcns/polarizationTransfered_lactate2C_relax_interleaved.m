function[fitness] = polarizationTransfered_lactate2C_relax_interleaved(population,pars)
tic
...
T1_I = 1117;                       % 13C longitudinal
T2_I = 1110.3;                        % 13C transverse
T1_S = 11111.73;                        % 1H longitudinal
...
for i = npop:-1:1
    ...
    for ff = 1%:length(fRange)
        ...
        H_sup_free = 2*pi*deltaf*H_sup_offres + 2*pi*methylOffset*H_sup_Sz_CH3 + ...                %frequency offset
...
%                 rho(16) = 1;
                ...
                for j = 1:6:nvar
                    ...
                    H_sup = 2*pi*gamma_H*B1_S*(cos_pulse_phase(k)*H_sup_Sx + sin_pulse_phase(k)*H_sup_Sy) + ...  %pulse on proton
                        2*pi*deltaf*H_sup_offres + 2*pi*methylOffset*H_sup_Sz_CH3 + ...                         %frequency offset
                        2*pi*H_sup_Jcoupling;                                                                %coupling
