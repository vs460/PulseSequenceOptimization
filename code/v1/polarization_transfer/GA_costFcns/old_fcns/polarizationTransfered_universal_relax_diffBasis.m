...
genH_sup = false;
if ~genH_sup
path = 'C:\Users\Somai01\Documents\pulseSequenceDesign\INEPT_optimization\lactate';
    ...
    H_sup_Ix = gpuArray(H_sup_Ix);
    H_sup_Iy = gpuArray(H_sup_Iy);
    H_sup_Iz = gpuArray(H_sup_Iz);
    H_sup_Sx = gpuArray(H_sup_Sx);
    H_sup_Sy = gpuArray(H_sup_Sy);
    H_sup_Sz = gpuArray(H_sup_Sz);
    H_sup_IzSz = gpuArray(H_sup_IzSz);
end
...
J = 5.67;                           % coupling in Hz
B1_range = 0.9:0.05:1.1;                  % pulse amplitude range for compensation
fRange = -2:2:2;                   % offresonance range for compensation
...
for i = 1:npop
    ...
    signal = zeros(length(B1_range),length(B1_range),'gpuArray');
    contamination = 0;
    bb_C = 1;
    for B1_C = B1_range
        bb_H = 1;
        ...
        for deltaf = fRange
            ...
            for j = 1:5:nvar
                ...
                H_sup = (2*pi*gamma_C*B1_L*(cos(pulse_phase_L)*H_sup_Ix + sin(pulse_phase_L)*H_sup_Iy) +...        %pulse on carbon
                         ...
                         2*pi*deltaf*(H_sup_Iz + gamma_H/gamma_C*H_sup_Sz) + ...                                          %frequency offset
                         2*pi*J*(H_sup_IzSz));                                                                     %coupling
                H_sup = -1i*H_sup - R;
                rho = expm(H_sup*tau)*rho;
                %rho = rho/sum(abs(rho))*3;
            end
    %        densityMTX = zeros(2^nspins,2^nspins);
    %        for rr = 1:length(rho)
    %            densityMTX = densityMTX + rho(rr)*(L(:,:,1,permutation_mtx(rr,1))*L(:,:,2,permutation_mtx(rr,2))*L(:,:,3,permutation_mtx(rr,3))*L(:,:,4,permutation_mtx(rr,4)));
    %        end
    %        H_free = 2*pi*deltaf*(Iz + gamma_H/gamma_C*Sz) + ...           % frequency offset
    %        2*pi*J*(Iz*Sz);                                             % coupling
    %       time_step = 1/norm(H_free);
    %        P_evolution = expm(-1i*H_free*time_step);
    %        nsteps = 4096;
    %        for fd = 1:nsteps 
    %            fid(fd) = trace(coil'*densityMTX);                          % Detection = trace ... projection of the density on the detection state
    %           densityMTX = P_evolution*densityMTX*P_evolution';
    %       end
    %        apodisation = exp(-5*linspace(0,1,nsteps));
    %       fid = fid.*apodisation;
    %       spectrum = fftshift(fft(fid));
    %        signal = signal + spectrum;
            signal_tmp = signal_tmp + (rho(2)+1i*rho(3));
            %contamination = contamination+rho(254)+rho(255)+rho(65)+rho(66)+rho(206)+rho(207)+rho(242)+rho(243)+rho(194)+rho(195)+rho(53)+rho(54)+rho(20)+rho(21);
        end
    ...
    fitness(i)	= 1e3*gather(-sum(abs(signal)));
end
toc
end
