function[fitness] = JimmuneRF_lac2C_approx(population,pars)
tic
...
minPw_I        = pars.minPw_I;            % minimum Xnuc pulse width to achieve frequency selectivity
minPw_S        = pars.minPw_S;            % minimum 1H pulse width to achieve frequency selectivity
...
B1weight_X = normpdf(B1_range_X,1,3/2.355);
...
% **********************************************************
n_of_I_spins = 1;
n_of_S_spins = 1; % this can be set to 1 as J_CH >> any other J-coupling
...
%%%%% this should be uncommented if all 4 protons are accounted for %%%%%
...
%%%%% uncomment if all protons are accounted for %%%%%
Sx          = Sx_CH;%+Sx_CH3;
Sy          = Sy_CH;%+Sy_CH3;
Sz          = Sz_CH;%+Sz_CH3;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Jnegl       = J_HH3*Sz_CHSz_CH3 + J_CH3*IzSz_CH3;
%%%%% uncomment +Jnegl if all protons are accounted for %%%%%
Jcoupling   = J_CH*Iz*Sz;%+Jnegl; % use magnetic equivalence and secular approximation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Idmtx       = eye(2^nspins);

%% initialization
initState   = Iz;
traceNorm   = trace(Iz'*Iz);      % normalize everything to the initial state
coil_iph    = Ix+1i*Iy;           % in-phase signal state
coil_aph    = 2*(Sz*Ix+1i*Sz*Iy); % anti-phase signal state

%% fitness evaluation
fitness  = zeros(npop,1);
parfor i = 1:npop
    parameters = (population(i,1:nvar));
    taus_tmp   = min(max(0,parameters(3:3:end)),1/abs(J_CH));
    % enforce smoothness constraint on the pulse amplitude    
    B1_I_vec   = min(round(abs(parameters(1:3:end-2))/min_C_amp),1).*sign(parameters(1:3:end-2)).*max(min(abs(parameters(1:3:end-2)),pars.X_maxAmp),min_C_amp)
    ...
    for ff = 1:length(fRange)
        ...
        %%%%% uncomment if all 4 protons accounted for %%%%%
        %2*pi*metOffset*Sz_CH3 ;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        bb_C     = 1;
        for B1_C = B1_range_X
            WBC  = B1weight_X(bb_C);
            ...
            %%%%%%%%%% Pulse sequence %%%%%%%%%%
            for j = 1:3:nvar
                B1_I  = B1_C*B1_I_vec((j-1)/3+1);
                ph    = parameters(j+1);
                tau   = min(max(0,parameters(j+2)),1/abs(J_CH));
                
                %%% pulse %%%
                H     = 2*pi*B1_I*(cos(ph)*Ix + sin(ph)*Iy)+H_free;                                                          %coupling
                PH    = expm(-1i*H*tau);
                PHt   = PH';
            ...
            weight                = WBC*Wf;
            sg_tmp_iph            = trace(coil_iph'*rho)/traceNorm; % in-phase signal component
            sg_tmp_aph            = trace(coil_aph'*rho)/traceNorm; % anti-phase signal component
            signal(bb_C,ff)       = weight*(imag(sg_tmp_iph));
            % penalty term for the difference of in-phase and anti-phase signal
            asymmetry(bb_C,ff)    = weight*2e0*abs(sg_tmp_iph-sg_tmp_aph);
            sum_weight            = sum_weight + weight;
            bb_C = bb_C + 1;
        end
    end
    signal_tred = sum(sum(signal))/sum_weight;
    penalty = sum(sum(asymmetry))/sum_weight;
    % calculating the result's frequency profile and penalising deviation
    % from prescribed, i.e. non-zero excitation of pyruvate and PH
    cost = calcRFprofileNMRstuff(B1_I_vec,parameters,taus_tmp,pars,0);
    fitness(i)	= 1e3*gather(-abs(signal_tred)+penalty);
end
toc
end
