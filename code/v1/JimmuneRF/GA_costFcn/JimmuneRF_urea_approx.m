function[fitness] = JimmuneRF_urea_approx(population,pars)

tic
[npop,nvar]    = size(population);
gamma_N        = pars.gX;                 % Xnuc gyromagnetic ratio
gamma_H        = pars.gH;                 % proton gyromagnetic ratio
...
min_N_amp      = pars.min_X_amp;          % a minimum aplitude can be set if required
N_maxAmp       = pars.X_maxAmp;           % maximum B1 amplitude to comply with transmit chain
...
B1weight_X     = normpdf(B1_range_X,1,1.5/2.355);
...
Sx1 = L(:,:,n_of_I_spins+1,2);
Sy1 = L(:,:,n_of_I_spins+1,3);
Sz1 = L(:,:,n_of_I_spins+1,4);

Sx2 = L(:,:,n_of_I_spins+2,2);
Sy2 = L(:,:,n_of_I_spins+2,3);
Sz2 = L(:,:,n_of_I_spins+2,4);
...
% use magnetic equivalence and secular approximation
Jcoupling = J_NH*Iz*Sz; 
...
coil_aph  = -4*(Ix+1i*Iy)*Sz1*Sz2; % anti-phase signal state
%% fitness evaluation
fitness = zeros(npop,1);
parfor i = 1:npop
    parameters = (population(i,1:nvar));
    taus_tmp   = min(max(0,parameters(3:3:end)),1/abs(J_NH));
    % enforce smoothness constraint on the pulse amplitude
    B1_I_vec   = min(round(abs(parameters(1:3:end-2))/min_N_amp),1).*sign(parameters(1:3:end-2)).*max(min(abs(parameters(1:3:end-2)),pars.X_maxAmp),min_N_amp);
    ...
    contamination = zeros(lB1r_X,lB0r);
    ...
    for ff = 1:length(fRange)
        ...
        for B1_C = B1_range_X
            ...
            for j = 1:3:nvar
                ...
                ph   = parameters(j+1);
                tau  = min(max(0,parameters(j+2)),1/abs(J_NH));
                
                %%% pulse %%%
                H    = 2*pi*B1_I*(cos(ph)*Ix + sin(ph)*Iy)+H_free;                
                PH   = expm(-1i*H*tau);
                PHt  = PH';
            ...
            WBC                    = B1weight_X(bb_C);
            weight                 = Wf*WBC;
            sg_tmp_iph             = trace(coil_iph'*rho)/traceNorm; % in-phase signal term
            sg_tmp_aph             = trace(coil_aph'*rho)/traceNorm; % anti-phase signal term
            signal(bb_C,ff)        = weight*2*((sg_tmp_iph));
            % penalty term for the difference of in-phase and anti-phase signal
            asymmetry(bb_C,ff)     = weight*1e1*abs(sg_tmp_iph-sg_tmp_aph);
            % penalty term for mixed anti-phase terms not to end up with
            % asymmetric doublet
            contamination(bb_C,ff) = weight*(trace((coil_iph*Sz)'*rho)/traceNorm);
    ...
    cont        = sum(sum(contamination))/sum_weight;    
    fitness(i)	= 1e3*gather(-abs(signal_tred)+penalty+abs(cont));
end
toc
end
