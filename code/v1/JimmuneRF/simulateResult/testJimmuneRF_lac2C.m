...
B1_range_X    = linspace(0,3,15);        % pulse amplitude range for compensation
fRange        = linspace(-100,100,30);   % offresonance range for compensation
...
for ff = 1:length(fRange)
    ...
    H_free  = 2*pi*df*offres + 2*pi*metOffset*Sz_CH3 + ...        
        2*pi*Jcoupling;
    parfor bb_C = 1:length(B1_range_X)
        ...
        %%%%%%%%%% Pulse sequence %%%%%%%%%%
        for j = 1:nvar
            % putting together the pulse point
            B1_I = B1_C*RF.Amp(j);
            ph   = RF.Phase(j);
            tau  = RF.Taus(j);
...
pars.signal  = signal;
avgSignal    = sum(sum(signal))/sum_weight % ensemble average signal
